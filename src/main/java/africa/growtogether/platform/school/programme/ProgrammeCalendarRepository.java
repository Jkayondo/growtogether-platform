...
/**
 * Shared read-only Today Programme projection over the authoritative
 * academic-calendar domain.
 *
 * Calendar recurrence_rule is currently reserved/unimplemented by the
 * academic-calendar write path. This query therefore reads only
 * materialised persisted event intervals and does not invent recurrence
 * expansion semantics.
 */
public interface ProgrammeCalendarRepository
        extends Repository<AcademicCalendarEvent, UUID> {

    /**
     * Returns displayable academic-calendar events overlapping one
     * school-local day.
     *
     * Interval semantics:
     *
     ...
     *
     * Lifecycle visibility excludes DRAFT, POSTPONED, CANCELLED and
     * ARCHIVED. COMPLETED remains visible because an event completed
     * earlier today is still part of today's programme/history.
     */
    @Query("""
            select e

...
public class SecurityConfiguration {
    ...
    SecurityFilterChain securityFilterChain(
        ...
        TenantBoundaryFilter tenantBoundaryFilter,
    ...
    /*
     * Security filters are managed exclusively by Spring Security.
     *
     * Both filters are Spring beans for dependency injection, but must not
     * also be auto-registered independently with the servlet container.
     * Otherwise they can execute twice and outside the intended
    ...
    @Bean
    FilterRegistrationBean<TenantBoundaryFilter>
    tenantBoundaryFilterRegistration(
        TenantBoundaryFilter filter
    ) {
        FilterRegistrationBean<TenantBoundaryFilter> registration =
            new FilterRegistrationBean<>(filter);
    ...
    SecretKey jwtSecretKey(
        ...
        if(bytes.length < 32){
            ...
            throw new IllegalStateException(
                "GT JWT secret must be at least 32 bytes."
            );
